...
export default function Home() {
  ...
  const paymentCounts = React.useMemo(() => {
    const list = ownerDashboard?.rent_status || [];
    return {
      all: list.length,
      overdue: list.filter((r) => r.payment_status === "overdue").length,
      pending: list.filter((r) => r.payment_status === "pending" || r.payment_status === "upcoming").length,
      paid: list.filter((r) => r.payment_status === "paid").length,
      partial: list.filter((r) => r.payment_status === "partial").length,
    };
  }, [ownerDashboard?.rent_status]);

  const sortedAndFilteredTenants = React.useMemo(() => {
    if (!ownerDashboard?.rent_status) return [];

    let filtered = ownerDashboard.rent_status;

    // Search filter: tenant name, email, property name, unit name
    if (paymentSearchTerm.trim()) {
      const term = paymentSearchTerm.toLowerCase();
      filtered = filtered.filter(
        (rent) =>
          rent.tenant_name.toLowerCase().includes(term) ||
          (rent.tenant_email && rent.tenant_email.toLowerCase().includes(term)) ||
          rent.property_name.toLowerCase().includes(term) ||
          rent.unit_name.toLowerCase().includes(term)
      );
    }
    ...
    if (paymentStatusFilter !== "all") {
      if (paymentStatusFilter === "pending") {
        filtered = filtered.filter(
          (rent) => rent.payment_status === "pending" || rent.payment_status === "upcoming"
        );
      } else {
        filtered = filtered.filter((rent) => rent.payment_status === paymentStatusFilter);
      }
    }
    ...
    // Sort grouped tenants
    return grouped.sort((a, b) => {
      const aTotalDue = a.rents.reduce((sum, r) => sum + r.total_due, 0);
      const bTotalDue = b.rents.reduce((sum, r) => sum + r.total_due, 0);
      const aTotalPending = a.rents.reduce((sum, r) => sum + r.pending, 0);
      const bTotalPending = b.rents.reduce((sum, r) => sum + r.pending, 0);
      const aOverdueCount = a.rents.filter((r) => r.payment_status === "overdue").length;
      const bOverdueCount = b.rents.filter((r) => r.payment_status === "overdue").length;

      switch (paymentSortOption) {
        case "tenant-name-asc":
          return a.tenantName.localeCompare(b.tenantName);
        case "tenant-name-desc":
          return b.tenantName.localeCompare(a.tenantName);
        case "pending-amount-desc":
          return bTotalPending - aTotalPending;
        case "pending-amount-asc":
          return aTotalPending - bTotalPending;
        case "total-due-desc":
          return bTotalDue - aTotalDue;
        case "overdue-count-desc":
          return bOverdueCount - aOverdueCount;
        default:
          return 0;
      }
    });
  }, [ownerDashboard?.rent_status, paymentSearchTerm, paymentStatusFilter, paymentSortOption]);
              ...
              <div className="flex flex-wrap gap-2 border-b border-[#e2e8f0] pb-3">
                ...
                >
                  ...
                  <span className={`rounded-full px-1.5 py-0.2 text-[10px] ${
                    paymentStatusFilter === "all" ? "bg-white/20 text-white" : "bg-slate-200 text-[#475569]"
                  }`}>
                    {paymentCounts.all}
                  </span>
                </button>
                ...
                <button
                  ...
                  className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-semibold transition-all cursor-pointer ${
                    paymentStatusFilter === "overdue"
                      ...
                      : "bg-[#fee2e2] text-[#991b1b] hover:bg-[#fca5a5]/30"
                  }`}
                >
                  Overdue
                  <span className={`rounded-full px-1.5 py-0.2 text-[10px] ${
                    paymentStatusFilter === "overdue" ? "bg-white/20 text-white" : "bg-[#fca5a5] text-[#991b1b]"
                  }`}>
                ...
                <button
                  ...
                  className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-semibold transition-all cursor-pointer ${
                    paymentStatusFilter === "pending"
                      ...
                      : "bg-[#fef3c7] text-[#92400e] hover:bg-[#fde68a]/30"
                  }`}
                >
                  Pending
                  <span className={`rounded-full px-1.5 py-0.2 text-[10px] ${
                    paymentStatusFilter === "pending" ? "bg-white/20 text-white" : "bg-[#fde68a] text-[#92400e]"
                  }`}>
                ...
                >
                  Partial
                  <span className={`rounded-full px-1.5 py-0.2 text-[10px] ${
                    paymentStatusFilter === "partial" ? "bg-white/20 text-white" : "bg-[#fde68a] text-[#b45309]"
                  }`}>
                ...
                <button
                  ...
                  className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-semibold transition-all cursor-pointer ${
                    paymentStatusFilter === "paid"
                      ...
                      : "bg-[#dcfce7] text-[#166534] hover:bg-[#bbf7d0]/30"
                  }`}
                >
                  Paid
                  <span className={`rounded-full px-1.5 py-0.2 text-[10px] ${
                    paymentStatusFilter === "paid" ? "bg-white/20 text-white" : "bg-[#bbf7d0] text-[#166534]"
                  }`}>
                    {paymentCounts.paid}
                  </span>
                </button>
